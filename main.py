...
LABEL_STUDIO_DATA_PATH = ""
yolov5_repo_dir = r"yolov5"
pretrained_model_path = r"yolov5.pt"
model = custom(path=pretrained_model_path)
...
class DummyModel(LabelStudioMLBase):
    ...
    def predict(self, tasks, **kwargs):
        ...
        for i, task in enumerate(tasks):
            print("original path:  ", task['data']['image'])

            
            image_path = str(task['data']['image'])
            if ("local-files" in image_path):
                new_path = "/data/local-files/" + image_path[image_path.index("?d=")+3:]
            else:
                new_path = image_path
            print("new path:  ", new_path)
            img = cv2.imread(os.path.join(new_path), cv2.COLOR_BGR2RGB)
            ...
            for elem in labels:
                ...
                cls = int(elem[-1])
                if (str(names[cls]) in self.labels):
    ...
    def fit(self, completions, workdir=None, **kwargs):
        ...
        print("Sorry training is not yet implemented!")
        pass
        return {}
